...
DOM structure (confirmed via live investigation 2025):
  ...
  section._2x6hy p                       → evening speed detail ("down XX Mbps & up XX Mbps")
  section._3r4CW ul li                   → feature bullet points

Card layout per plan (confirmed from probe output):
  4G Internet  : 90-Day,  $54.90 / $49.90,  total $149.70,  20 Mbps,  4G network
  Bronze       : 12-month, $70.90 / $58.90,  total $58.90,  25 Mbps,  NBN
  Silver       : 12-month, $80.90 / $70.90,  total $70.90,  50 Mbps,  NBN
  Gold         : 12-month, $85.90 / $71.90,  total $71.90,  99 Mbps,  NBN
  Gold Plus    : 12-month, $85.90 / $71.90,  total $71.90, 500 Mbps,  NBN
  Platinum     : 12-month, $94.90 / $84.90,  total $84.90, 740 Mbps,  NBN
  Diamond      : 12-month, $108.90/ $94.90,  total $94.90, 850 Mbps,  NBN
"""
...
def _parse_total_cost(note: str) -> Optional[float]:
    """
    Extract total cost from strings like:
...
def _extract_card(card: ElementHandle, index: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # price_monthly = the promotional price (what customers actually pay)
        # price_once_off = the standard rate ("thereafter" price)
        ...
        # ── Validation ──────────────────────────────────────────────────────
        if not plan['name'] or not plan['speed'] or plan['price_monthly'] <= 0:
            log_warning(
                f'Card {index} incomplete — name={plan["name"]!r} '
                f'speed={plan["speed"]!r} price={plan["price_monthly"]}',
                provider=PROVIDER,
            )
            return None
...
def scrape_kogan_plans() -> List[Dict[str, Any]]:
            ...
            for i, card in enumerate(plan_cards):
                ...
                if plan is not None:
                    ...
                    log_info(
                        f'  [{i}] {plan["name"]:12s}  '
                        f'{plan["speed"]:10s}  '
                        f'${plan["price_monthly"]:.2f}/mth  '
                        f'(full ${plan["price_full"]:.2f})  '
                        f'total ${plan["total_cost"]:.2f}',
                        provider=PROVIDER,
                    )
    ...
    all_plans.sort(key=lambda x: x.get('speed_down', 0))
...
if __name__ == '__main__':
    ...
    print(SEP)
    print(f'{"#":<3} {"Plan Name":<12} {"Type":<6} {"Duration":<12} '
          f'{"Speed":>8}  {"Promo/mth":>10}  {"Full/mth":>9}  '
          f'{"Total":>9}  {"Features"}')
    print('-' * 110)

    for p in plans:
        feats = '; '.join(p['features'])[:55] if p['features'] else '—'
        print(
            f'{p["index"]:<3} '
            f'{p["name"]:<12} '
            f'{p["network_type"]:<6} '
            f'{p["plan_duration"]:<12} '
            f'{p["speed"]:>8}  '
            f'${p["price_monthly"]:>8.2f}  '
            f'${p["price_full"]:>7.2f}  '
            f'${p["total_cost"]:>7.2f}  '
            f'{feats}'
        )
