"""WaveZone provider scraper.

Scrapes fibre broadband month-to-month plans from https://wavezone.net/.
"""
...
PROVIDER_ID = config.PROVIDERS.get('wavezone', {}).get('id', 26)
PROVIDER_NAME = 'WaveZone'
URL = 'https://wavezone.net/'
...
def _fetch_html() -> str:
    # WaveZone may fail certificate validation in Python's cert store on Windows.
...
def _extract_plans_from_lines(lines: List[str]) -> List[Dict[str, Any]]:
    ...
    while i < len(lines):
        ...
        if not download_speed:
            log_warning(f'WaveZone: missing speed data for {name}', provider='wavezone')
            i += 1
            continue
...
def scrape_wavezone_plans() -> List[Dict[str, Any]]:
    log_info('Starting WaveZone scraper', provider='wavezone')
    plans: List[Dict[str, Any]] = []
    ...
    try:
        ...
        if not pricing_section:
            log_error('WaveZone: #pricingarea section not found', provider='wavezone')
            return []
        ...
        log_error(f'WaveZone scraper failed: {exc}', provider='wavezone')
    ...
    if not unique_plans:
        log_warning('WaveZone scraper returned no plans', provider='wavezone')
    log_success(f'WaveZone scraper complete: {len(unique_plans)} plans', provider='wavezone')
    return unique_plans


if __name__ == '__main__':
    for item in scrape_wavezone_plans():
        print(item)
